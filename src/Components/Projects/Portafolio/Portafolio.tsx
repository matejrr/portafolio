...
import PortafolioHeader from "./ProgressHeader";
...
const Portafolio: React.FC<ProjectSectionProps> = ({
    reverse = false,
    cardIndex,
}) => {
    const portfolioData = projectsData.find(
        (project) => project.projectName === "Portfolio"
    );

    return (
        <Card key={cardIndex}>
            <PortafolioHeader heading={projectSections.portafolio} />
            <SectionContainer className="bg-[#1F2A34]" reverse={reverse}>
                <HeaderContainer>
                    <div className="flex flex-1 max-w-[40%]">
                        <SubHeader>{portfolioData?.projectName}</SubHeader>
                    </div>
                    <span className="text-2xl md:text-3xl font-bold uppercase tracking-wider text-white border-b-2 border-cyan-400 pb-2 mb-6">
                        {portfolioData?.finished}
                    </span>
                </HeaderContainer>
                <ContentWrapper>
                    <TextWrapper>{portfolioData?.description}</TextWrapper>
                    <TextWrapper>{portfolioData?.explanation}</TextWrapper>

                    <ImageWrapper></ImageWrapper>
                </ContentWrapper>
            </SectionContainer>
            <div className="flex flex-row gap-4">
                {portfolioData?.images?.techStack.map((image, index) => (
