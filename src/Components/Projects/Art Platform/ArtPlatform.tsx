...
import OnlineGalleryHeader from "./ProgressHeader";
...
const ArtPlatform: React.FC<ProjectSectionProps> = ({
    reverse = false,
    cardIndex,
}) => {
    const artPlatformData = projectsData.find(
        (project) => project.projectName === "Art Platform"
    );

    return (
        <Card key={cardIndex}>
            <OnlineGalleryHeader heading={projectSections.artPlatform} />
            <SectionContainer className="bg-[#1F2A34]" reverse={reverse}>
                <HeaderContainer>
                    <div className="flex flex-1 max-w-[40%]">
                        <SubHeader>{artPlatformData?.projectName}</SubHeader>
                    </div>
                    <span className="text-2xl md:text-3xl font-bold uppercase tracking-wider text-white border-b-2 border-cyan-400 pb-2 mb-6">
                        {artPlatformData?.finished}
                    </span>
                </HeaderContainer>
                <ContentWrapper>
                    <TextWrapper>{artPlatformData?.description}</TextWrapper>
                    <TextWrapper>{artPlatformData?.explanation}</TextWrapper>

                    <ImageWrapper></ImageWrapper>
                </ContentWrapper>
            </SectionContainer>
            <div className="flex flex-row gap-4">
                {artPlatformData?.images?.techStack.map((image, index) => (
